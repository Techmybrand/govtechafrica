...
//             CONTACT_EMAIL: email,
//             submitType: "optinCustomView",
//             lD: "116e7f964d9c550ba",
//             emailReportId: "",
//             zx: "1371db337",
//             zcvers: "3.0",
//             oldListIds: "",
//             mode: "OptinCreateView",
//             zcld: "116e7f964d9c550ba",
//             zctd: "116e7f964d9c2db89",
//             document_domain: "",
...
export async function POST(req: Request) {
    try {
        ...
        const zohoRes = await fetch(
            "https://dcqorm-cmpzourl.maillist-manage.com/weboptin.zc",
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
        ...
        if (normalizedResponse.includes("already a part of our organization") || normalizedResponse.includes("already signed up") || normalizedResponse.includes("already subscribed")) {
            return NextResponse.json({error: "This email address has already been subscribed."}, { status: 409 });
        }
        return NextResponse.json({message: "Successfully subscribed!"}, { status: 200 });
    } catch (error) {
