...
const ArticlesView = () => {
	...
	const articles = sortedBlogs.filter((blog: BlogDetailsProps) => blog.type.toLowerCase() === "article");
	useEffect(() => {
		fetchBlogs();
				...
				{!articles?.length ? (
					...
					<div className={styles.research_wrapper}>
						{articles?.map((blog: BlogDetailsProps, index: number) => {
							return (
								<ResearchCard key={index} title={blog?.title} image={`https:${blog?.thumbnail?.fields?.file?.url}`}
									slug={blog?.slug} publishedAt={blog?.publishedAt} researchType={blog?.type}
